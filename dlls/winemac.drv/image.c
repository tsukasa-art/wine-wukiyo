                      ...
                      struct gdi_image_bits *bits, struct bitblt_coords *src)
{
     ...
     * CGWindowListCreateImage is marked unavailable in macOS 15+ headers but the
     * runtime symbol is still present.  Resolve via dlsym to avoid the compile-time
     * error.  TODO: migrate to ScreenCaptureKit when Wine gains screen-capture
     * entitlement support. */
    {
        typedef CGImageRef (*fn_t)(CGRect, CGWindowListOption, CGWindowID, CGWindowImageOption);
        static fn_t fn;
        if (!fn) fn = (fn_t)dlsym(RTLD_DEFAULT, "CGWindowListCreateImage");
        if (!fn) goto fallback;
        cgimage = fn(CGRectNull,
