                            ...
                            LARGE_INTEGER *offset, ULONG *key )
{
    ...
    if (!(!status && type == FD_TYPE_FILE &&
          (options & (FILE_SYNCHRONOUS_IO_ALERT | FILE_SYNCHRONOUS_IO_NONALERT)) &&
          virtual_is_arm64ec_low_buffer( buffer, length )) &&
        !virtual_check_buffer_for_write( buffer, length )) return STATUS_ACCESS_VIOLATION;
    ...
    if (type == FD_TYPE_FILE)
    {
        ...
        if (offset && offset->QuadPart != FILE_USE_FILE_POINTER_POSITION)
        {
            ...
            while ((result = (async_read ? virtual_locked_pread( unix_handle, buffer, length, offset->QuadPart ) :
                       virtual_locked_arm64ec_file_io( unix_handle, buffer, length, offset->QuadPart, TRUE, FALSE ))) == -1)
            {
                if (errno != EINTR)
                {
                    status = errno_to_status( errno );
    ...
    for (;;)
    {
        if ((result = (type == FD_TYPE_FILE && !async_read ?
                       virtual_locked_arm64ec_file_io( unix_handle, (char *)buffer + total, length - total, 0, FALSE, FALSE ) :
                       virtual_locked_read( unix_handle, (char *)buffer + total, length - total ))) >= 0)
        {
            total += result;
                             ...
                             LARGE_INTEGER *offset, ULONG *key )
{
    ...
    if (type == FD_TYPE_FILE)
    {
        ...
        if (offset && offset->QuadPart != FILE_USE_FILE_POINTER_POSITION)
        {
            ...
            /* async I/O doesn't make sense on regular files */
            while ((result = (async_write ? pwrite( unix_handle, buffer, length, off ) :
                       virtual_locked_arm64ec_file_io( unix_handle, (void *)buffer, length, off, TRUE, TRUE ))) == -1)
            {
                if (errno != EINTR)
                {
    ...
    for (;;)
    {
        result = type == FD_TYPE_FILE && !async_write ?
            virtual_locked_arm64ec_file_io( unix_handle, (char *)buffer + total, length - total, 0, FALSE, TRUE ) :
            write( unix_handle, (const char *)buffer + total, length - total );
