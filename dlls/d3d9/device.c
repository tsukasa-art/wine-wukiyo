 ...
 * thumbnails via GetBackBuffer + LockRect(READONLY, full rect) on the
 * implicit swapchain back buffer, once per save/load screen open.  No GDI,
 * no GetRenderTargetData, no full-res StretchRect involved.  The content is
        ...
        const RECT *src_rect, IDirect3DSurface9 *dst_surface, const RECT *dst_rect, D3DTEXTUREFILTERTYPE filter)
{
    ...
    if (!swingby_rt_capture_active)
    {
        ...
        {
            static DWORD s_sr192_throttle = 0;
    ...
    struct d3d9_device *device, struct d3d9_surface *src_impl)
{
    ...
    if (FAILED(d3d9_device_CreateOffscreenPlainSurface(iface, sd.Width, sd.Height,
            sd.Format, D3DPOOL_SYSTEMMEM, &sys, NULL)))
        return;
    sys_impl = unsafe_impl_from_IDirect3DSurface9(sys);

    SetRect(&r, 0, 0, sd.Width, sd.Height);
    wined3d_mutex_lock();
    hr = wined3d_device_context_blt(device->immediate_context,
            sys_impl->wined3d_texture, sys_impl->sub_resource_idx, &r,
            src_impl->wined3d_texture,  src_impl->sub_resource_idx, &r,
...
static HRESULT WINAPI d3d9_device_SetRenderTarget(IDirect3DDevice9Ex *iface, DWORD idx, IDirect3DSurface9 *surface)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        /* Diag: log RT changes (deduplicated) to see which surface the scene
         * and the save-screen UI are composed into. */
        if (idx < 4)
        {
