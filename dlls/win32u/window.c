                                          ...
                                          LONG_PTR newval, BOOL ansi, BOOL internal )
{
    ...
    WNDPROC class_proc = NULL;
    LONG_PTR requested_proc = newval;
    STYLESTRUCT style;
    WND *win;
    ...
    case GWLP_WNDPROC:
        if (win->class) class_proc = (WNDPROC)get_class_long_ptr( hwnd, GCLP_WNDPROC, ansi );
    ...
    {
        retval = (ULONG_PTR)get_window_proc( (WNDPROC)retval, old_ansi, ansi );
        /* Preserve the self-referencing subclass guard with server-owned wndproc state. */
        if (retval && retval == requested_proc && class_proc)
            retval = (ULONG_PTR)class_proc;
    }
    return retval;
}
